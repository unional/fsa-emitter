...
export class Emitter {
	...
	emit<Payload, Meta>({ type, payload, meta, error }: FSA<string, Payload, Meta>) {
		...
		this.listenAlls.forEach((l) => {
			l({ type, payload, meta, error })
		})
		if (this.listenMisses.length > 0 && this.emitter.listeners(type).length === 0)
			this.listenMisses.forEach((l) => {
				l({ type, payload, meta, error })
			})
	}
	/**
	...
	addListener<Payload, Meta>(
		...
		if (type === errorEvent.type) return this.addErrorEventListener(listener)
	...
	queue<Payload, Meta>(
		...
		const wrap = (payload: Payload, meta: Meta, error: boolean) => {
			listener(payload, meta, error)
			queue.shift()
			if (queue.length > 0) this.emitter.once(type, queue[0])
		}

		queue.push(wrap)
		if (queue.length === 1) return this.emitter.once(type, wrap)
	...
	onAny(listener: (fsa: FluxStandardAction<any, any, any>) => void) {
		...
		return {
			remove: () => {
				...
				if (i >= 0) this.listenAlls.splice(i, 1)
			}
		}
	}
	...
	onMissed(listener: (fsa: FluxStandardAction<any, any, any>) => void) {
		...
		return {
			remove: () => {
				...
				if (i >= 0) this.listenMisses.splice(i, 1)
			}
		}
	}
