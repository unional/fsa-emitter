...
test('no emit', () => {
	const noEmit = createEventAction('noEmit', () => () => {
		return
	})
	const emitter = new Emitter()
	emitter.on(noEmit, () => {
...
test('create event action', () => {
	const minus = createEventAction<{ a: number; b: number }, { a: number; b: number; result: number }>(
		'minus',
		({ a, b }) =>
			(emit) => {
				emit({ a, b, result: a + b })
...
test('match should type guard an action', () => {
	const noop = createEventAction('noop', () => () => {
		return
	})
