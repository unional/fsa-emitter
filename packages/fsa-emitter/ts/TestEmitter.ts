...
export class TestEmitter extends Emitter {
	...
	addListener<Payload, Meta>(
		...
		if (type === errorEvent.type) return this.addErrorEventListener(wrap)
	...
	listenedTo(events: (TypedEvent<any, any> | string)[] | { [k: string]: TypedEvent<any, any> }) {
		if (Array.isArray(events)) return this.listenedToEventArray(events)
...
function buildDefaultMiss(log: { error: (...args: any[]) => void }) {
  ...
  type: ${type}
  payload: ${tersify(payload, { maxLength: Number.POSITIVE_INFINITY })}
  meta: ${tersify(meta)}`)
	}
}
